...
ingest this chat last one was getting slow Fresh start. I will upload one file: app.py (and presets.json if needed). Goal: minimal fixes for my phone-call sim. I do NOT want to edit code myself — give me a full drop-in app.py. Requirements: - SR = 48000 end-to-end. - Modern tiers = ("high","ultra_high"): • DO NOT apply legacy bandwidth clamps. • DO NOT apply handset IR. • In apply_phone_quality_tier: ultra_high bandwidth = (20, SR/2); use zero_phase for modern tiers. - Landline tiers = ("good_landline","bad_landline","cordless"): • Add μ-law color via a parallel band-limited path (300–2400 Hz), amounts: 0.2 (good), 0.35 (bad/cordless). • No μ-law on modern tiers. - IR double guard: if room_ir_file == bg_ir_file (abspath), skip one convolve to avoid doubling. - Keep the UI and existing features; no new features. Deliverable: - Return a COMPLETE replacement app.py (single code block). No prose, no placeholders.
