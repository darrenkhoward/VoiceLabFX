...
def apply_phone_quality_tier(y: np.ndarray, tier: str, custom_params: dict = None) -> tuple[np.ndarray, str]:
    ...
    tiers = {
        ...
        "bad_landline": {
            ...
            "mu_law_intensity": 0.0,       # Disable μ-law to prevent distortion
            "noise_level": 0.003,          # Reduced steady hiss/hum
            "dropout_boost": 0.3,          # Occasional crackles (not digital)
            "garble_boost": 0.0,           # No digital garble
        ...
        "cordless": {
            ...
            "mu_law_intensity": 0.0,       # Disable μ-law to prevent distortion
            "noise_level": 0.002,          # Reduced RF noise (use RF slider for more)
            "dropout_boost": 0.5,          # Range interference
            "garble_boost": 0.2,           # Slight digital artifacts
        ...
        "low": {
            ...
            "garble_boost": 1.0,           # Normal garble
        ...
        "standard": {
            ...
            "mu_law_intensity": 0.0,       # Disable μ-law to prevent distortion
            "noise_level": 0.001,          # Reduced noise (was 0.002)
            "dropout_boost": 0.8,          # Fewer dropouts
            "garble_boost": 0.8,           # Less garble
        ...
        "high": {
            ...
            "dropout_boost": 0.3,          # Very few dropouts
            "garble_boost": 0.3,           # Very little garble
...
def load_presets() -> Dict[str, Any]:
    def _blank():
        return {
            ...
            "presets": {
                ...
                "🎯 True Source": {
                    "dereverb": 0.0, "src_hpf": 0, "src_lpf": 24000, "leveler_amt": 0.0, "wpe_strength": 0.0,
                    "room_ir_file": None, "room_ir_gain_db": 0, "quality_tier": "ultra_high",
    ...
    try:
        ...
        defaults = _blank()["presets"]
        for name, preset in defaults.items():
            if name not in j["presets"]:
                j["presets"][name] = preset
    ...
    with gr.Blocks(title="VoiceLab FX — Editor", theme=theme) as demo:
        ...
        with gr.Tab("Phone Quality"):
            ...
            quality_tier = gr.Radio(
                choices=["good_landline", "bad_landline", "cordless", "ultra_low", "low", "standard", "high", "ultra_high", "custom"],
                value="standard",
                label="Quality Tier",
                info="Select predefined quality level or 'custom' for manual control"
            )
            ...
            gr.Markdown("**Network Artifacts** *(auto-update with tier selection)*")
            ...
            with gr.Row():
                garble_p  = gr.Slider(0.0, 0.5, 0.02, step=0.01, label="Garble Probability", info="Resampling-based warble for poor signal (0-0.5 range)")
                stutter_a = gr.Slider(0.0, 0.08, 0.0, step=0.001, label="Stutter Amount", info="Cellular dropout repetition (0-0.08 range)")
            with gr.Row():
                jitter_a  = gr.Slider(0.0, 0.3, 0.0, step=0.01, label="Jitter Intensity", info="VoIP timing variations (10-80ms wobble)")
                buffer_p  = gr.Slider(0.0, 1.0, 0.0, step=0.01, label="Buffer Probability", info="Rebuffering smears (70-300ms)")
            with gr.Row():
                reorder_p = gr.Slider(0.0, 0.5, 0.0, step=0.01, label="Packet Reorder", info="Packets arrive out of order")
                codec_type = gr.Dropdown(["amr_nb", "amr_wb", "opus", "evs"], value="amr_nb", label="Codec Type", visible=False)
                codec_intensity = gr.Slider(0.0, 1.0, 0.0, step=0.01, label="Codec Artifacts", info="Codec-specific distortions", visible=False)
            with gr.Row():
            ...
            with gr.Row():
                ...
                rf_a      = gr.Slider(0.0, 1.0, 0.00, step=0.01, label="RF Noise", info="Cellular/cordless only - disabled for landlines")
